...
public class NameSurferGraph extends GCanvas implements NameSurferConstants, ComponentListener {
	...
	// Class constructor
	public NameSurferGraph(HashMap hashmap) {
		addComponentListener(this);
		dataSize = hashmap.size();
	}
	...
	private void drawNameUserEntries() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (double n = 0; n < NDECADES - 1; n++) {
				...
				double y0 = 20 + ((getHeight() - 40) * (rank / dataSize));

				double x1 = getWidth() * ((n + 1) / NDECADES);
				double y1 = 20 + ((getHeight() - 40) * (rankNPlus1 / dataSize));
	...
	//The parameter passed is the same as that passed to update() above, expect it is referenced from
	//within this class rather than p[assed as a parameter by another class
